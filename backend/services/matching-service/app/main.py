...
from shared.models.tables import (
    User, SkillProfile, CandidateRoleFit, RoleTemplate,
    InitialProfile, TrustSignal, TrustLevel, BiasAuditLog,
    CandidateAssignment, InterviewSession
)
...
async def _mock_parse_resume(file_bytes: bytes) -> dict:
    ...
    return {
        ...
        "extracted_text_preview": "Senior Software Engineer with 4 years...",
...
async def parse_and_enrich_resume(
    user_id: str,
    ...
    if profile:
        ...
        profile = InitialProfile(
            ...
            resume_text=parsed_data["extracted_text_preview"],
            years_experience=parsed_data["years_experience"],
            skills=list(parsed_data["skills"].keys()),
    ...
    if not skill_profile:
        ...
        skill_profile = SkillProfile(
            ...
            proficiency_levels=parsed_data["skills"]
        )
...
async def match_candidate_to_role(
    ...
    fit = db.query(CandidateRoleFit).filter(
        CandidateRoleFit.candidate_id == user_id_uuid,
        CandidateRoleFit.role_template_id == role_id_uuid
    ).first()

    if not fit:
        fit = CandidateRoleFit(
            candidate_id=user_id_uuid,
            role_template_id=role_id_uuid
        )
        db.add(fit)
...
async def generate_trust_signal(
    data: GenerateTrustSignalRequest,
    db: Session = Depends(get_db)
):
...
async def log_bias_audit(
    ...
    log = BiasAuditLog(
        ...
        role_template_id=UUID(role_id),
        demographic_cohort=cohort_hash,
        overall_score=score,
        outcome=outcome
    )
    db.add(log)
...
async def get_audit_stats(
    ...
    for c, stats in cohort_stats.items():
        results.append(BiasAuditStatsResponse(
            cohort=c,
            average_score=round(stats["score_sum"] / stats["total"], 2),
            pass_rate=round(stats["passed"] / stats["total"], 2),
