...
async def generate_text_ollama(prompt: str, model: str) -> str:
    ...
    data = {'model': model, 'prompt': prompt, 'stream': False, 'options': {'temperature': 0.7, 'top_p': 0.9, 'max_tokens': 256}}
...
async def generate_text_gemini(prompt: str) -> str:
    ...
    for attempt in range(len(GEMINI_API_KEYS)):
        try:
            ...
            completion = await asyncio.to_thread(model.generate_content, prompt)
...
async def generate_text_openai(system_prompt: str, user_message: str, history: List[Dict]) -> str:
    try:
        ...
        completion = await asyncio.to_thread(client.chat.completions.create, model='gpt-4o', messages=messages, temperature=0.7, max_tokens=512)
...
async def generate_response(req: GenerateRequest):
    try:
        ...
        return GenerateResponse(response_text=response_text, audio_url=audio_url, question_type='follow_up', phase_suggestion='continue')
    except Exception as e:
        print(f'AI Generation Error: {e}')
        return GenerateResponse(response_text="I apologize, but I'm having trouble retrieving my thoughts right now. Could you repeat that?", question_type='error')
...
async def transcribe(req: TranscribeRequest):
    try:
        ...
        if not groq_client:
            return TranscribeResponse(transcript='', error='Groq API key not configured. Set GROQ_API_KEY in .env')
        audio_file = io.BytesIO(audio_data)
        audio_file.name = 'audio.webm'
        try:
            result = await asyncio.to_thread(groq_client.audio.transcriptions.create, model='whisper-large-v3', file=audio_file, language='en')
            ...
            try:
                ...
                result = await asyncio.to_thread(groq_client.audio.transcriptions.create, model='whisper-large-v3', file=wav_buf, language='en')
                transcript = result.text.strip() if result.text else ''
                print(f"Groq Whisper (wav fallback) transcribed: '{transcript[:80]}'")
                return TranscribeResponse(transcript=transcript)
            except Exception as wav_err:
                print(f'STT wav conversion also failed: {wav_err}')
                return TranscribeResponse(transcript='', error=f'Transcription failed: {str(wav_err)}')
    except Exception as e:
...
@app.get('/health')
async def health():
    return {'status': 'healthy', 'service': 'speech-pipeline', 'stt_provider': 'groq-whisper', 'tts_provider': 'edge-tts', 'groq_configured': groq_client is not None, 'gemini_keys': len(GEMINI_API_KEYS)}
_WHITEBOARD_MODE_PROMPTS = {'explain': 'Explain the following in a clear, concise way suitable for a technical interview. Use plain language and avoid unnecessary jargon. If relevant, give a brief example.', 'code': 'Provide a clean, working code solution for the following. Add brief inline comments. Return ONLY the code block with no extra prose.', 'hints': 'Give 3-5 progressive hints (from small nudge to bigger hint) to help solve the following problem without giving away the direct answer.', 'solution': 'Provide a complete, step-by-step solution for the following problem. Include: 1) approach/algorithm, 2) code implementation, 3) time/space complexity analysis.'}
...
async def whiteboard_ask(req: WhiteboardRequest):
    try:
        ...
        if not ai_text and settings.openai_api_key and ('placeholder' not in settings.openai_api_key.lower()):
            try:
                ai_text = await generate_text_openai('You are a technical interview coach helping a candidate.', full_prompt, [])
            except Exception as e:
        ...
        return WhiteboardResponse(response=ai_text, format=fmt, timestamp=datetime.utcnow().isoformat())
    except Exception as e:
        print(f'Whiteboard endpoint error: {e}')
        return WhiteboardResponse(response='AI is temporarily unavailable. Please try again.', format='text', timestamp=datetime.utcnow().isoformat())
