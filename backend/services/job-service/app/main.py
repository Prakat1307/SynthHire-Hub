...
from backend.shared.models.tables import JobListing, JobApplication, JobSource, JobStatus, ApplicationStatus, User, RoleTemplate, Certificate, CompanyAccount
from backend.shared.schemas.job import JobListingCreate, JobListingUpdate, JobListingResponse, JobApplicationCreate, JobApplicationResponse
...
class GeminiKeyRotator:
    ...
    def __init__(self):
        ...
        raw_keys = [settings.gemini_api_key, settings.gemini_api_key_2, settings.gemini_api_key_3, settings.gemini_api_key_4]
...
def create_job_listing(data: JobListingCreate, user_id: str=Depends(get_current_user_id), db: Session=Depends(get_db)):
    ...
    job = JobListing(company_id=company.id, role_template_id=data.role_template_id, title=data.title, description=data.description, requirements=data.requirements, location=data.location, is_remote=data.is_remote, salary_range=data.salary_range, expires_at=data.expires_at, status=JobStatus.PUBLISHED, published_at=datetime.datetime.utcnow())
    db.add(job)
    db.commit()
    db.refresh(job)
    return job
...
async def search_jobs(q: Optional[str]=None, remote_only: bool=False, company_id: Optional[UUID]=None, user_id: Optional[str]=Depends(get_current_user_id), db: Session=Depends(get_db)):
    query = db.query(JobListing).filter(JobListing.status == JobStatus.PUBLISHED)
    if q:
        search = f'%{q}%'
        query = query.filter(or_(JobListing.title.ilike(search), JobListing.description.ilike(search)))
    if remote_only:
    ...
    result = []
    for job in jobs:
        j_dict = {'id': job.id, 'title': job.title, 'description': job.description, 'requirements': job.requirements, 'location': job.location, 'is_remote': job.is_remote, 'salary_range': job.salary_range, 'external_url': job.external_url, 'company_id': job.company_id, 'role_template_id': job.role_template_id, 'source_id': job.source_id, 'status': job.status, 'published_at': job.published_at, 'expires_at': job.expires_at, 'created_at': job.created_at, 'updated_at': job.updated_at, 'company_name': job.company.name if job.company else 'External Company', 'match_score': match_scores.get(str(job.id))}
...
def get_job_details(job_id: UUID, db: Session=Depends(get_db)):
    ...
    j_dict = {'id': job.id, 'title': job.title, 'description': job.description, 'requirements': job.requirements, 'location': job.location, 'is_remote': job.is_remote, 'salary_range': job.salary_range, 'external_url': job.external_url, 'company_id': job.company_id, 'role_template_id': job.role_template_id, 'source_id': job.source_id, 'status': job.status, 'published_at': job.published_at, 'expires_at': job.expires_at, 'created_at': job.created_at, 'updated_at': job.updated_at, 'company_name': job.company.name if job.company else 'External Company', 'match_score': None}
    return j_dict

@app.post('/jobs/{job_id}/apply', response_model=JobApplicationResponse)
def apply_to_job(job_id: UUID, data: JobApplicationCreate, user_id: str=Depends(get_current_user_id), db: Session=Depends(get_db)):
    job = db.query(JobListing).filter(JobListing.id == job_id).first()
    if not job:
        raise HTTPException(status_code=404, detail='Job not found')
    existing = db.query(JobApplication).filter(JobApplication.job_id == job_id, JobApplication.candidate_id == UUID(user_id)).first()
    if existing:
        raise HTTPException(status_code=400, detail='Already applied to this job')
    if data.certificate_id:
        cert = db.query(Certificate).filter(Certificate.id == data.certificate_id, Certificate.user_id == UUID(user_id)).first()
        if not cert:
            raise HTTPException(status_code=400, detail='Invalid certificate')
    app = JobApplication(job_id=job_id, candidate_id=UUID(user_id), certificate_id=data.certificate_id, cover_letter=data.cover_letter, status=ApplicationStatus.NEW)
    db.add(app)
    db.commit()
    db.refresh(app)
    return {**app.__dict__, 'job_title': job.title, 'company_name': job.company.name if job.company else 'External'}

@app.get('/applications/me', response_model=List[JobApplicationResponse])
def get_my_applications(user_id: str=Depends(get_current_user_id), db: Session=Depends(get_db)):
    apps = db.query(JobApplication).filter(JobApplication.candidate_id == UUID(user_id)).all()
    result = []
    for a in apps:
        result.append({**a.__dict__, 'job_title': a.job.title if a.job else 'Unknown', 'company_name': a.job.company.name if a.job and a.job.company else 'External'})
    return result
...
async def aggregate_jobs(query: JobSearchQuery, db: Session=Depends(get_db)):
    jobs = await JobAggregationService.fetch_jobs(query=query.query, location=query.location, page=query.page, db=db)
...
async def calculate_ai_match(request: MatchRequest, request_obj: Request=None, db: Session=Depends(get_db)):
    resume_text = None
        ...
        try:
            ...
            if auth_header.startswith('Bearer ') and auth_header != 'Bearer null':
                ...
                try:
                    ...
                    if uid:
                        resume = db.query(Resume).filter(Resume.user_id == UUID(uid), Resume.is_active == True).first()
                        if not resume:
    ...
    try:
        ...
        return {'match_score': result.get('match_score', 0), 'details': {'strengths': result.get('key_strength_matches', []), 'weaknesses': result.get('missing_critical_skills', []), 'summary': result.get('brief_summary', '')}}
    except Exception as e:
...
async def auto_apply_job(request: ApplicationRequest, user_id: str=Depends(get_current_user_id), db: Session=Depends(get_db)):
    from backend.shared.models.tables import ResumeVersion as Resume
    existing = db.query(JobApplication).filter(JobApplication.job_id == request.job_id, JobApplication.user_id == UUID(user_id)).first()
    if existing:
    ...
    user_profile = {'name': user.full_name if user else 'John Doe', 'email': user.email if user else 'test@example.com'}
    resume_path = None
    try:
        auto_apply = AutoApplyService(api_key=settings.gemini_api_key)
        results = await auto_apply.execute_apply(job_url=request.job_url, user_profile=user_profile, resume_path=resume_path)
        if results.get('status') == 'success':
            app = JobApplication(job_id=request.job_id, user_id=UUID(user_id), status=ApplicationStatus.APPLIED.value)
            db.add(app)
            db.commit()
