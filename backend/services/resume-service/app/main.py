...
from .schemas import ATSAnalyzeRequest, ATSAnalyzeResponse, BulletRewriteRequest, BulletRewriteResponse, CoverLetterRequest, CoverLetterResponse, ResumeOptimizeRequest, ResumeOptimizeResponse, PrepEvaluateRequest, PrepEvaluateResponse, PrepJdAnalyzeRequest, PrepJdAnalyzeResponse, PrepHistoryResponse, PrepDraftRequest, PrepDraftResponse, LinkedInOptimizeRequest, LinkedInOptimizeResponse, LinkedInKeywordMatchRequest, LinkedInKeywordMatchResponse, LinkedInFetchRequest, LinkedInFetchResponse
...
def get_service(db: Session=Depends(get_db)) -> ResumeService:
    return ResumeService(db=db, gemini_api_key=settings.gemini_api_key, gemini_model=settings.gemini_model)

@app.post('/resume/analyze', response_model=ATSAnalyzeResponse)
async def analyze_ats(data: ATSAnalyzeRequest, user_id: str=Depends(get_current_user_id), service: ResumeService=Depends(get_service)):
    try:
...
async def rewrite_bullet(data: BulletRewriteRequest, user_id: str=Depends(get_current_user_id), service: ResumeService=Depends(get_service)):
    try:
...
async def generate_cover_letter(data: CoverLetterRequest, user_id: str=Depends(get_current_user_id), service: ResumeService=Depends(get_service)):
    try:
        result = await service.generate_cover_letter(user_id, data.job_description, data.company_name, data.hiring_manager or 'Hiring Manager', data.angle)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=f'Cover letter generation failed: {str(e)}')

@app.post('/resume/optimize', response_model=ResumeOptimizeResponse)
async def optimize_resume(data: ResumeOptimizeRequest, user_id: str=Depends(get_current_user_id), service: ResumeService=Depends(get_service)):
    try:
...
async def evaluate_prep_answer(data: PrepEvaluateRequest, user_id: str=Depends(get_current_user_id), service: ResumeService=Depends(get_service)):
    try:
...
async def generate_prep_draft(data: PrepDraftRequest, user_id: str=Depends(get_current_user_id), service: ResumeService=Depends(get_service)):
    try:
...
async def analyze_prep_jd(data: PrepJdAnalyzeRequest, user_id: str=Depends(get_current_user_id), service: ResumeService=Depends(get_service)):
    try:
...
async def get_prep_history(question_id: str, user_id: str=Depends(get_current_user_id), service: ResumeService=Depends(get_service)):
    try:
...
async def linkedin_optimize(data: LinkedInOptimizeRequest, user_id: str=Depends(get_current_user_id), service: ResumeService=Depends(get_service)):
    try:
        result = await service.linkedin_optimize(section=data.section, resume_data=data.resume_data, target_role=data.target_role, jd=data.job_description, existing=data.existing_linkedin_text)
        return result
    except Exception as e:
        import traceback
        traceback.print_exc()
...
async def linkedin_keyword_match(data: LinkedInKeywordMatchRequest, user_id: str=Depends(get_current_user_id), service: ResumeService=Depends(get_service)):
    try:
        result = await service.linkedin_keyword_match(resume_skills=data.resume_skills, jd_keywords=data.jd_keywords, linkedin_skills=data.linkedin_skills)
        return result
    except Exception as e:
        import traceback
        traceback.print_exc()
...
async def linkedin_fetch_profile(data: LinkedInFetchRequest, user_id: str=Depends(get_current_user_id), service: ResumeService=Depends(get_service)):
    try:
