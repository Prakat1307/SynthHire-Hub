...
class ResumeService:
    ...
    def _get_resume_text(self, user_id: str, resume_version_id: str = None) -> str:
        ...
        if resume_version_id:
            resume = self.db.query(ResumeVersion).filter(
    ...
    def _get_user_stories(self, user_id: str) -> str:
        ...
        stories = self.db.query(PersonalStory).filter(
            PersonalStory.user_id == user_id,
            PersonalStory.is_active == True,
        ).all()
        if not stories:
            return ""
    ...
    async def generate_cover_letter(
        self, user_id: str, job_description: str, company_name: str,
        hiring_manager: str = "Hiring Manager", angle: str = "balanced"
    ) -> dict:
    ...
    async def optimize_resume(self, user_id: str, resume_version_id: str, job_description: str) -> dict:
        ...
        existing_count = self.db.query(ResumeVersion).filter(
            ResumeVersion.user_id == user_id,
        ).count()
        ...
        new_version = ResumeVersion(
            user_id=user_id,
            label=f"Tailored: {jd_label}...",
            is_master=False,
            raw_text=optimized_text,
            parsed_json=optimization,
            tailored_for_jd=job_description,
...
"""
        ...
        history_record = PrepAnswerHistory(
            user_id=user_id,
            question_id=data.question[:250], 
            answer_text=data.answer,
            scores_json=parsed.get("scores", {}),
...
"""
    ...
    async def get_prep_history(self, user_id: str, question_id: str) -> dict:
        history = self.db.query(PrepAnswerHistory).filter(
            PrepAnswerHistory.user_id == user_id,
            PrepAnswerHistory.question_id == question_id[:250]
        ).order_by(PrepAnswerHistory.created_at.desc()).all()
...
"""
    ...
    async def linkedin_optimize(self, section: str, resume_data: str, target_role: Optional[str] = None, jd: Optional[str] = None, existing: Optional[str] = None) -> dict:
        ...
        section_prompts = {
            "headline": f"Generate a highly optimized LinkedIn headline (max 220 chars) for a candidate targeting {target}. Use this resume data: {resume_data}. Include current role/status + top skills + value prop. Make it ATS friendly. {jd_context}. {existing_context}",
            "about": f"Write a LinkedIn About section (max 2600 chars) targeting {target}. 3 paragraphs: 1) Who they are (hook), 2) What they do (key skills/experience), 3) What they are looking for. Storytelling tone. Use resume: {resume_data}. {jd_context}. {existing_context}",
            "experience": f"Rewrite this experience entry or infer one from the resume for LinkedIn targeting {target} (max 2000 chars). Use bullet points starting with action verbs, add quantified achievements. Use resume: {resume_data}. {jd_context}. {existing_context}",
            "skills": f"Extract top 15-20 comma-separated skills from this resume for a LinkedIn Skills section targeting {target}. Prioritize technical depth and JD relevance. Resume: {resume_data}. {jd_context}. {existing_context}",
            "projects": f"Rewrite project descriptions for LinkedIn targeting {target} (max 2000 chars). Format: [Project name] - [Summary]. 3-4 bullets on tech used and impact. Resume: {resume_data}. {jd_context}. {existing_context}",
            "open_to_work": f"Suggest up to 5 comma-separated job titles for LinkedIn's 'Open To Work' feature. Based on resume: {resume_data} and target: {target}. {jd_context}"
        }
