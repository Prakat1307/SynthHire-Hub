...
from .schemas import (
    ResumeParseResponse, ResumeVersionResponse, JobTargetingRequest,
    PersonalStoryCreate, PersonalStoryUpdate, PersonalStoryResponse,
    PreferencesRequest, PreferencesResponse, OnboardingStatusResponse,
)
...
app = FastAPI(
    title="SynthHire Onboarding Service",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def save_job_targeting(
    data: JobTargetingRequest,
    user_id: str = Depends(get_current_user_id),
    service: OnboardingService = Depends(get_service),
):
...
async def create_story(
    ...
    return {
        ...
        "role_at_time": story.role_at_time,
...
async def save_preferences(
    ...
    return {
        "id": str(prefs.id),
        "tone": prefs.tone.value if prefs.tone else "conversational",
        "answer_length": prefs.answer_length.value if prefs.answer_length else "medium",
        "answer_format": prefs.answer_format.value if prefs.answer_format else "star",
