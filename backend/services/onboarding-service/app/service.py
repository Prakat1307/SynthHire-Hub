...
from shared.models.tables import (
    User, UserProfile, PersonalStory, UserPreferences, ResumeVersion,
    AnswerTone, AnswerLength, AnswerFormat
)
...
class OnboardingService:
    ...
    async def parse_resume(self, user_id: str, raw_text: str, file_name: str = None) -> dict:
        ...
        existing_master = self.db.query(ResumeVersion).filter(
        ...
        version_number = 1
        if existing_master:
            max_version = self.db.query(ResumeVersion).filter(
                ResumeVersion.user_id == user_id
            ).count()
        ...
        resume = ResumeVersion(
            user_id=user_id,
            label="Master Resume" if not existing_master else f"Resume v{version_number}",
            is_master=not existing_master,  
            raw_text=raw_text,
            parsed_json=parsed,
            file_name=file_name,
    ...
    def get_resume_versions(self, user_id: str) -> List[ResumeVersion]:
        ...
        ).order_by(ResumeVersion.is_master.desc(), ResumeVersion.created_at.desc()).all()
    ...
    def create_story(self, user_id: str, data: dict) -> PersonalStory:
        ...
        count = self.db.query(PersonalStory).filter(
    ...
    def get_stories(self, user_id: str) -> List[PersonalStory]:
        return self.db.query(PersonalStory).filter(
            PersonalStory.user_id == user_id,
            PersonalStory.is_active == True
        ).order_by(PersonalStory.sort_order).all()

    def update_story(self, user_id: str, story_id: str, data: dict) -> PersonalStory:
        story = self.db.query(PersonalStory).filter(
            PersonalStory.id == story_id,
            PersonalStory.user_id == user_id,
        ).first()
        if not story:
    ...
    def save_preferences(self, user_id: str, data: dict) -> UserPreferences:
        ...
        tone_map = {"formal": AnswerTone.FORMAL, "conversational": AnswerTone.CONVERSATIONAL,
                     "assertive": AnswerTone.ASSERTIVE, "warm": AnswerTone.WARM}
        length_map = {"short": AnswerLength.SHORT, "medium": AnswerLength.MEDIUM,
                      "detailed": AnswerLength.DETAILED}
        format_map = {"star": AnswerFormat.STAR, "bullet_points": AnswerFormat.BULLET_POINTS,
                      "flowing": AnswerFormat.FLOWING}
        ...
        if prefs:
            ...
            prefs = UserPreferences(
                user_id=user_id,
                tone=tone_map.get(data.get("tone", "conversational"), AnswerTone.CONVERSATIONAL),
                answer_length=length_map.get(data.get("answer_length", "medium"), AnswerLength.MEDIUM),
                answer_format=format_map.get(data.get("answer_format", "star"), AnswerFormat.STAR),
            )
            self.db.add(prefs)
    ...
    def get_onboarding_status(self, user_id: str) -> dict:
        ...
        resume_count = self.db.query(ResumeVersion).filter(
            ResumeVersion.user_id == user_id,
            ResumeVersion.is_active == True
        ).count()

        stories_count = self.db.query(PersonalStory).filter(
            PersonalStory.user_id == user_id,
            PersonalStory.is_active == True
        ).count()
        ...
        return {
            ...
            "job_targeting_done": job_targeting_done,
            "stories_count": stories_count,
            "preferences_set": prefs is not None,
        }