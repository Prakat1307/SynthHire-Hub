...
from shared.models.tables import (
    CompanyAccount, RoleTemplate, ScoringProfile,
    CandidateAssignment, AssignmentStatus, ATSIntegration, User
)
...
async def _trigger_ats_webhook(db: Session, company_id: UUID, event_type: str, payload: dict):
    ...
    integrations = db.query(ATSIntegration).filter(
        ATSIntegration.company_id == company_id,
...
async def process_assignment(
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                match_res = await client.post(
                    "http://matching-service:8015/match",
                    json={"candidate_id": str(assignment.candidate_id), "role_id": str(role.id)},
                    timeout=5.0
                )
    ...
    if role.company_id:
        ...
        background_tasks.add_task(
            _trigger_ats_webhook,
            db, role.company_id,
            "candidate_assessed" if not auto_shortlisted else "candidate_shortlisted",
            payload
        )

    return ProcessAssignmentResponse(
        assignment_id=str(assignment.id),
...
async def get_candidate_ranking(
    ...
    assignments = db.query(CandidateAssignment).filter(
        ...
        CandidateAssignment.status.in_([AssignmentStatus.ASSESSED, AssignmentStatus.SHORTLISTED, AssignmentStatus.PENDING])
    ).all()

    sorted_assignments = sorted(
        assignments,
        key=lambda a: (
            a.status.value == "shortlisted",  
            a.weighted_score or 0.0,
            a.fit_score or 0.0,
            a.overall_score or 0.0
        ),
        reverse=True
    )
...
async def batch_shortlist(
    data: BatchShortlistRequest,
    background_tasks: BackgroundTasks,
    user_id: str = Depends(get_current_user_id),
    db: Session = Depends(get_db)
):
    ...
    assignments = db.query(CandidateAssignment).filter(
        CandidateAssignment.id.in_([UUID(id) for id in data.assignment_ids]),
        CandidateAssignment.status == AssignmentStatus.ASSESSED
    ).all()
    ...
    for a in assignments:
        ...
        if role and role.company_id:
            ...
            company_payloads[role.company_id].append({
                "assignment_id": str(a.id),
