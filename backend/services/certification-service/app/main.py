...
from shared.models.tables import Certificate, CertificateStatus, InterviewSession, SkillProfile, User, AssessmentMode
...
async def create_certificate(data: CertificateCreate, user_id: str=Depends(get_current_user_id), db: Session=Depends(get_db)):
    ...
    certificate = Certificate(user_id=UUID(user_id), session_id=session.id, certificate_code=generate_certificate_code(), title=session_to_certificate_title(session), role_type=session.target_role or 'General', difficulty_level=str(session.difficulty_level or 5), overall_score=session.overall_score, dimension_scores=session.dimension_scores or {}, status=CertificateStatus.ACTIVE, issued_at=now, expires_at=now + timedelta(days=settings.certificate_ttl_days), share_token=generate_share_token(), is_public=False, summary_text=f'Completed {session_to_certificate_title(session)} with an overall score of {session.overall_score:.1f}%', rubric_snapshot=session.dimension_scores)
...
async def list_certificates(user_id: str=Depends(get_current_user_id), status: Optional[str]=None, page: int=Query(1, ge=1), page_size: int=Query(20, ge=1, le=100), db: Session=Depends(get_db)):
    ...
    return CertificateListResponse(certificates=[_cert_to_response(c) for c in certs], total=total)

@app.get('/certificates/{certificate_id}', response_model=CertificateResponse)
async def get_certificate(certificate_id: str, user_id: str=Depends(get_current_user_id), db: Session=Depends(get_db)):
    cert = db.query(Certificate).filter(Certificate.id == UUID(certificate_id), Certificate.user_id == UUID(user_id)).first()
    if not cert:
        raise HTTPException(status_code=404, detail='Certificate not found')
    return _cert_to_response(cert)

@app.put('/certificates/{certificate_id}/visibility')
async def toggle_visibility(certificate_id: str, is_public: bool=True, user_id: str=Depends(get_current_user_id), db: Session=Depends(get_db)):
    cert = db.query(Certificate).filter(Certificate.id == UUID(certificate_id), Certificate.user_id == UUID(user_id)).first()
    if not cert:
        raise HTTPException(status_code=404, detail='Certificate not found')
...
async def verify_certificate(share_token: str, db: Session=Depends(get_db)):
    ...
    return CertificatePublicView(certificate_code=cert.certificate_code, title=cert.title, role_type=cert.role_type, difficulty_level=cert.difficulty_level, overall_score=cert.overall_score, dimension_scores=cert.dimension_scores or {}, status=cert.status.value, issued_at=cert.issued_at.isoformat() if cert.issued_at else '', expires_at=cert.expires_at.isoformat() if cert.expires_at else '', summary_text=cert.summary_text, candidate_name=candidate_name)
...
def _cert_to_response(cert: Certificate) -> CertificateResponse:
    return CertificateResponse(id=str(cert.id), certificate_code=cert.certificate_code, title=cert.title, role_type=cert.role_type, difficulty_level=cert.difficulty_level, overall_score=cert.overall_score, dimension_scores=cert.dimension_scores or {}, status=cert.status.value if cert.status else 'active', issued_at=cert.issued_at.isoformat() if cert.issued_at else '', expires_at=cert.expires_at.isoformat() if cert.expires_at else '', share_url=cert.share_url, is_public=cert.is_public or False, view_count=cert.view_count or 0, summary_text=cert.summary_text)

def _check_expirations(db: Session, user_id: str):
    now = datetime.now(timezone.utc)
    expired = db.query(Certificate).filter(Certificate.user_id == UUID(user_id), Certificate.status == CertificateStatus.ACTIVE, Certificate.expires_at < now).all()
    for cert in expired:
        cert.status = CertificateStatus.EXPIRED
    if expired:
        db.commit()
...
def _update_skill_profile(db: Session, user_id: str, session: InterviewSession):
    ...
    for dim_name, field_name in [('technical_correctness', 'technical_knowledge'), ('problem_solving', 'problem_solving'), ('communication', 'communication'), ('code_quality', 'coding_correctness'), ('system_design', 'system_design'), ('behavioral', 'behavioral_fit'), ('leadership', 'leadership'), ('culture_fit', 'culture_fit')]:
    ...
    history.append({'date': datetime.now(timezone.utc).isoformat(), 'overall': session.overall_score, 'dimensions': dim_scores, 'session_id': str(session.id)})
