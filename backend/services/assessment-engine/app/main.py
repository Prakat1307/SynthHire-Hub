...
GEMINI_API_KEYS = [k for k in [settings.gemini_api_key, settings.gemini_api_key_2, settings.gemini_api_key_3, settings.gemini_api_key_4] if k and 'placeholder' not in k]
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        ml_pipeline = MLInferencePipeline(deberta_model_path=settings.deberta_model_path if settings.deberta_model_path else None, embedding_model_name=settings.embedding_model_name, use_gpu=settings.use_gpu)
...
async def get_gpt4o_assessment(response_text: str, session_type: str, exchange_number: int, question_text: str=None) -> Dict[str, float]:
    question_context = f'Question: {question_text}' if question_text else ''
    try:
        client = openai.OpenAI(api_key=settings.openai_api_key)
        response = client.chat.completions.create(model='gpt-4', messages=[{'role': 'system', 'content': 'You are an expert interview assessor. Respond ONLY with valid JSON.'}, {'role': 'user', 'content': GPT4O_ASSESSMENT_PROMPT.format(session_type=session_type, question_context=question_context, response_text=response_text, exchange_number=exchange_number)}], temperature=0.3, max_tokens=200, response_format={'type': 'json_object'})
...
async def get_local_assessment(response_text: str, session_type: str, exchange_number: int, question_text: str=None) -> Dict[str, float]:
    question_context = f'Question: {question_text}' if question_text else ''
    ...
    try:
        data = {'model': settings.ollama_model, 'prompt': prompt, 'stream': False, 'format': 'json'}
...
async def get_gemini_assessment(response_text: str, session_type: str, exchange_number: int, question_text: str=None) -> Dict[str, float]:
    question_context = f'Question: {question_text}' if question_text else ''
    prompt = f'You are an expert interview assessor. Evaluate this candidate response across 10 dimensions.\n\nInterview Type: {session_type}\n{question_context}\n\nCandidate Response:\n"{response_text}"\n\nRate each dimension from 0.0 to 1.0:\n1. technical_correctness\n2. problem_decomposition\n3. communication_clarity\n4. handling_ambiguity\n5. edge_case_awareness\n6. time_management\n7. collaborative_signals\n8. growth_mindset\n9. semantic_quality\n10. problem_structure\n\nRespond ONLY with valid JSON: {{ "technical_correctness": 0.75, ...}} '
    last_error = None
...
async def assess_response(request: AssessRequest):
    ...
    if settings.use_local_ai:
        ai_scores = await get_local_assessment(response_text=request.response_text, session_type=request.session_type, exchange_number=request.exchange_number, question_text=request.question_text)
    ...
    if not ai_scores and GEMINI_API_KEYS:
        ai_scores = await get_gemini_assessment(response_text=request.response_text, session_type=request.session_type, exchange_number=request.exchange_number, question_text=request.question_text)
    if not ai_scores and settings.openai_api_key and ('placeholder' not in settings.openai_api_key):
        ai_scores = await get_gpt4o_assessment(response_text=request.response_text, session_type=request.session_type, exchange_number=request.exchange_number, question_text=request.question_text)
    if not ai_scores:
        ai_scores = {dim: 0.5 for dim in DIMENSION_NAMES}
    if ml_pipeline:
        result = ml_pipeline.score_response(candidate_response=request.response_text, gpt4o_scores=ai_scores, ideal_responses=request.ideal_responses, behavioral_scores=request.behavioral_scores)
        ...
        embedding_sim = None
        confidence = 0.6
    return AssessResponse(scores=final_scores, deberta_scores=deberta_scores, gpt4o_scores=ai_scores, embedding_similarity=embedding_sim, confidence=confidence)
...
async def generate_coaching_hints(request: Dict[str, Any]):
    ...
    if coaching_mode != 'training':
        return {'hints': []}
    hints = []
    thresholds = {'communication_clarity': (0.65, 'Structure your answer: approach first, then details'), 'edge_case_awareness': (0.55, 'Consider edge cases: empty input, maximum values'), 'problem_decomposition': (0.6, 'Break the problem into smaller sub-problems'), 'handling_ambiguity': (0.55, 'Ask clarifying questions before solving'), 'collaborative_signals': (0.6, 'Think out loud — share your reasoning'), 'time_management': (0.5, 'Start with a working solution, then optimize')}
    for dim, (threshold, hint_text) in thresholds.items():
        if scores.get(dim, 1.0) < threshold:
            hints.append({'hint': hint_text, 'dimension': dim, 'priority': 'high' if scores.get(dim, 1.0) < 0.4 else 'medium'})
    return {'hints': hints[:3]}

@app.get('/health')
async def health():
    return {'status': 'healthy', 'service': settings.service_name, 'port': settings.service_port, 'deberta_loaded': ml_pipeline is not None and hasattr(ml_pipeline, 'deberta') and (ml_pipeline.deberta is not None), 'embedding_loaded': ml_pipeline is not None and hasattr(ml_pipeline, 'embedding_scorer')}
