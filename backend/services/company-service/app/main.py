...
from shared.models.tables import (
    CompanyAccount, CompanyUser, CompanyRole,
    RoleTemplate, ScoringProfile, CandidateAssignment,
    ReviewAction, ReviewDecision, AssignmentStatus,
    ATSIntegration, User, UserAccountType,
    InterviewSession, SessionType
)
...
async def company_dashboard(
    ...
    if role_ids:
        assignments = db.query(CandidateAssignment).filter(
            CandidateAssignment.role_template_id.in_(role_ids)
        ).all()
    ...
    return DashboardStats(
        ...
        assessed=len([a for a in assignments if a.status and a.status.value == "assessed"]),
        shortlisted=len([a for a in assignments if a.status and a.status.value == "shortlisted"]),
        rejected=len([a for a in assignments if a.status and a.status.value == "rejected"]),
        pending=len([a for a in assignments if a.status and a.status.value == "pending"]),
    )
...
async def list_team_members(
    ...
    _require_company_member(db, user_id, company_id)
    members = db.query(CompanyUser).filter(
        CompanyUser.company_id == UUID(company_id),
        CompanyUser.is_active == True
    ).all()
...
async def create_role_template(
    ...
    role = RoleTemplate(
        ...
        description=data.description,
        interview_type=SessionType(data.interview_type) if data.interview_type else SessionType.TECHNICAL,
        duration_minutes=data.duration_minutes,
        scoring_profile_id=UUID(data.scoring_profile_id) if data.scoring_profile_id else None,
        auto_shortlist_enabled=data.auto_shortlist_enabled,
...
async def list_role_templates(
    ...
    roles = db.query(RoleTemplate).filter(
        RoleTemplate.company_id == UUID(company_id)
...
async def update_role_template(
    ...
    for field in ["title", "department", "experience_level", "description", "duration_minutes",
                   "auto_shortlist_enabled", "auto_shortlist_threshold"]:
...
async def create_scoring_profile(
    ...
    return ScoringProfileResponse(
        id=str(profile.id), name=profile.name, weights=profile.weights,
        thresholds=profile.thresholds, pass_threshold=profile.pass_threshold,
        is_default=profile.is_default or False,
    )
...
async def list_scoring_profiles(
    ...
    profiles = db.query(ScoringProfile).filter(
        ScoringProfile.company_id == UUID(company_id)
    ).all()
    return [ScoringProfileResponse(
        id=str(p.id), name=p.name, weights=p.weights,
        thresholds=p.thresholds, pass_threshold=p.pass_threshold,
...
async def list_candidates(
    ...
    query = db.query(CandidateAssignment).filter(
        CandidateAssignment.role_template_id == UUID(role_id)
    )
    if status:
...
async def get_candidate_detail(
    ...
    assignment = db.query(CandidateAssignment).filter(
        CandidateAssignment.id == UUID(assignment_id)
    ).first()
    if not assignment:
...
async def add_ats_integration(
    ...
    return {
        ...
        "is_active": ats.is_active,
    }

@app.get("/companies/{company_id}/ats")
async def list_ats_integrations(
    company_id: str,
    user_id: str = Depends(get_current_user_id),
    db: Session = Depends(get_db)
):
    _require_company_member(db, user_id, company_id)
    integrations = db.query(ATSIntegration).filter(
        ATSIntegration.company_id == UUID(company_id)
    ).all()
    return [{
        "id": str(i.id),
        "provider": i.provider,
        "webhook_url": i.webhook_url,
        "events_enabled": i.events_enabled or [],
...
{ 
    ...
    "interview_style": { 
        "culture": "collaborative/competitive/analytical",
...
Generate 3-5 rounds appropriate for {experience_level} level. Include coding, system design, and behavioral rounds as appropriate."""
        ...
        return {
            "rounds": [
                {"round_number": 1, "round_type": "coding", "title": "Technical Screen", "duration_minutes": 45,
                 "focus_areas": ["algorithms", "data_structures"], "sample_questions": ["Implement a data structure", "Solve an optimization problem"],
                 "persona_suggestion": "kind_mentor", "difficulty": 5},
                {"round_number": 2, "round_type": "system_design", "title": "System Design", "duration_minutes": 60,
                 "focus_areas": ["scalability", "architecture"], "sample_questions": ["Design a URL shortener", "Design a chat system"],
                 "persona_suggestion": "tough_lead", "difficulty": 6},
                {"round_number": 3, "round_type": "behavioral", "title": "Culture Fit", "duration_minutes": 30,
                 "focus_areas": ["teamwork", "leadership"], "sample_questions": ["Tell me about a time you disagreed", "Describe a difficult project"],
                 "persona_suggestion": "tricky_hr", "difficulty": 4},
            ],
            "tips": [f"Research {company_name}'s engineering blog", "Practice system design fundamentals"],
...
async def search_companies(
    ...
    if results:
        return [CompanySearchResult(
            ...
            industry=None, interview_style=c.interview_style,
            typical_rounds=c.typical_rounds, difficulty_level=c.difficulty_level,
            glassdoor_rating=c.glassdoor_rating, confidence_score=0.9, source="cache",
        ) for c in results]
    ...
    new_company = Company(
        name=llm_data.get("name", q),
        slug=slug,
        description=llm_data.get("description"),
        interview_style=llm_data.get("interview_style"),
        typical_rounds=llm_data.get("typical_rounds"),
        difficulty_level=llm_data.get("difficulty_level", "medium"),
        glassdoor_rating=llm_data.get("glassdoor_rating"),
    )
    try:
        db.add(new_company)
        db.commit()
    ...
    return [CompanySearchResult(
        name=llm_data.get("name", q), slug=slug,
        description=llm_data.get("description"),
        ...
        difficulty_level=llm_data.get("difficulty_level", "medium"),
        glassdoor_rating=llm_data.get("glassdoor_rating"),
        confidence_score=0.7, source="llm",
    )]
...
async def generate_blueprint(
    ...
    if company:
        ...
        company_data = {
            ...
            "typical_rounds": company.typical_rounds,
        }
...
def _company_to_response(company: CompanyAccount, db: Session) -> CompanyResponse:
    member_count = db.query(CompanyUser).filter(
        CompanyUser.company_id == company.id, CompanyUser.is_active == True
    ).count()
    active_roles = db.query(RoleTemplate).filter(
        RoleTemplate.company_id == company.id, RoleTemplate.is_active == True
    ).count()
    return CompanyResponse(
        ...
        plan=company.plan, member_count=member_count, active_roles=active_roles,
...
def _role_to_response(role: RoleTemplate) -> RoleTemplateResponse:
    return RoleTemplateResponse(
        id=str(role.id), title=role.title, department=role.department,
        experience_level=role.experience_level,
        interview_type=role.interview_type.value if role.interview_type else "technical",
        duration_minutes=role.duration_minutes or 45,
        is_active=role.is_active or True,
        candidates_assessed=role.candidates_assessed or 0,
        candidates_shortlisted=role.candidates_shortlisted or 0,
        auto_shortlist_enabled=role.auto_shortlist_enabled or False,
...
def _assignment_to_response(a: CandidateAssignment, candidate, reviews) -> CandidateAssignmentResponse:
    return CandidateAssignmentResponse(
        ...
        invited_at=a.invited_at.isoformat() if a.invited_at else "",
