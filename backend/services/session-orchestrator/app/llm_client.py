...
class InterviewLLMClient:
    ...
    def _build_system_prompt(self, persona_type: str, session_type: str, company_slug: Optional[str], persona_vector: Dict) -> str:
        ...
        base += '\nKeep your responses conversational and natural. You are voice-enabled, so speak naturally like a human interviewer. Do not use markdown formatting like bold or headers in your speech-to-text response.'
        return base
    ...
    async def send_message(self, session_id: str, user_message: str, context: Optional[Dict[str, Any]]=None) -> str:
        ...
        response_text = await self.client.generate_content(prompt=full_prompt, model_name=self.model)
