...
from shared.models.tables import (
    InterviewSession, SessionExchange, SessionStatus,
    SessionType, PersonaType, CoachingMode, Company
)
...
class SessionOrchestrator:
    ...
    def __init__(
        ...
        speech_client: ServiceClient,
        assessment_client: ServiceClient,
        coaching_client: ServiceClient,
        report_client: ServiceClient,
        code_executor_client: ServiceClient,
        emotion_client: ServiceClient,
    ...
    async def create_session(self, user_id: str, data: SessionCreate) -> InterviewSession:
        ...
        session = InterviewSession(
            ...
            coaching_mode=CoachingMode(data.coaching_mode),
            assessment_mode=AssessmentMode(data.assessment_mode) if hasattr(data, 'assessment_mode') else AssessmentMode.PRACTICE,
            difficulty_level=data.difficulty_level,
            company_slug=final_company_slug,
        ...
        await self.redis.set_session_state(str(session.id), {
            "status": "created",
            "user_id": user_id,
            ...
            "dimension_scores": {
                ...
                "handling_ambiguity": 0.0,
                "edge_case_awareness": 0.0,
                "time_management": 0.0,
                "collaborative_signals": 0.0,
                "growth_mindset": 0.0
            },
    ...
    async def load_session(self, session_id: str, user_id: str):
        ...
        self.difficulty_controller = AdaptiveDifficultyController(
            initial_difficulty=session.difficulty_level,
    ...
    async def start_session(self) -> Dict[str, Any]:
        ...
        await self.redis.append_conversation(str(self.session.id), {
            "role": "assistant",
            "content": opening_response["response_text"],
            "timestamp": datetime.utcnow().isoformat()
        })
        
        print("start_session: returning response")
        return {
            "response_text": opening_response["response_text"],
            "audio_url": opening_response.get("audio_url"),
            "exchange_number": 0,
            "phase": "AI_SPEAKING"
        }
    ...
    async def process_user_message(self, text: str) -> Dict[str, Any]:
        ...
        await self.redis.append_conversation(session_id, {
            "role": "user",
            "content": text,
            "timestamp": datetime.utcnow().isoformat()
        })
        ...
        if self.session.time_limit_minutes and elapsed_minutes > self.session.time_limit_minutes:
            return {
                ...
                "response_text": "The time limit for this assessment has been reached. Ending session.",
                "phase_suggestion": "end",
        ...
        behavioral_scores = {}
        try:
            beh_resp = await self.emotion_client.post("/emotion/behavioral-scores", {
        ...
        assessment_response = await self.assessment_client.post("/assess", {
            "session_id": session_id,
            "exchange_number": exchange_count,
            "response_text": text,
            "session_type": self.session.session_type.value,
        ...
        system_prompt = build_system_prompt(
            persona_type=self.session.persona_type.value,
            session_type=self.session.session_type.value,
            difficulty=difficulty_update["new_difficulty"],
            company_slug=self.session.company_slug,
            target_role=self.session.target_role,
        ...
        ai_response = await self.speech_client.post("/ai/generate", {
            "system_prompt": system_prompt,
            "conversation_history": history[-10:],
            "user_message": text,
            "persona_type": self.session.persona_type.value,
            "generate_audio": True,
            "difficulty_action": difficulty_update["action"]
        })
        ...
        await self.mongodb.save_exchange({
            "session_id": session_id,
            "exchange_number": exchange_count,
            "user_message": text,
            "ai_response": ai_response["response_text"],
            "dimension_scores": scores,
            "difficulty_at_time": difficulty_update["new_difficulty"],
            "timestamp": datetime.utcnow().isoformat()
        })
        ...
        exchange = SessionExchange(
            ...
            exchange_number=exchange_count,
            interviewer_text=ai_response["response_text"],
            candidate_text=text,
            dimension_scores=scores,
            difficulty_at_time=difficulty_update["new_difficulty"],
        ...
        if self.session.coaching_mode == CoachingMode.TRAINING:
            try:
                hints_response = await self.coaching_client.post("/coaching/hints", {
                    "scores": scores,
                    "session_type": self.session.session_type.value,
                    "exchange_number": exchange_count,
        ...
        return {
            ...
            "question_type": ai_response.get("question_type", "follow_up"),
            "phase_suggestion": ai_response.get("phase_suggestion", "continue"),
            "dimension_scores": scores,
            "coaching_hints": coaching_hints,
            "difficulty_level": difficulty_update["new_difficulty"]
        }

    async def process_audio(self, audio_base64: str) -> str:

        result = await self.speech_client.post("/speech/transcribe", {
            "audio": audio_base64
        })
    ...
    async def execute_code(self, code: str, language: str) -> Dict[str, Any]:
        ...
        result = await self.code_executor_client.post("/execute", {
            "code": code,
            "language": language,
            "session_id": str(self.session.id)
        })
    ...
    async def end_session(self) -> Dict[str, Any]:
        ...
        try:
            emo_summary = await self.emotion_client.post("/emotion/summary", {
                "session_id": session_id
            })
        ...
        try:
            await self.report_client.post("/generate", {
                "session_id": session_id
            }, headers={"X-User-Id": str(self.session.user_id)})
        ...
        return {
            ...
            "dimension_scores": self.session.dimension_scores,
