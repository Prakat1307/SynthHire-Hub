...
async def process_video_frame(request: VideoFrameRequest):
    ...
    try:
        ...
        if latest:
            return {'status': 'processed', 'confidence': latest.confidence_composite, 'dominant_emotion': latest.dominant_emotion, 'gaze_score': latest.gaze_score}
...
async def process_audio_chunk(request: AudioChunkRequest):
    ...
    try:
        ...
        if latest:
            return {'status': 'processed', 'confidence': latest.confidence_score, 'speech_rate_wpm': latest.speech_rate_wpm, 'pause_ratio': latest.pause_ratio}
        return {'status': 'processed'}
    except Exception as e:
        return {'status': 'error', 'detail': str(e)}
...
async def get_behavioral_scores(request: SessionSummaryRequest):
    ...
    if not pipeline:
        return {'scores': {dim: 0.5 for dim in ['technical_correctness', 'problem_decomposition', 'communication_clarity', 'handling_ambiguity', 'edge_case_awareness', 'time_management', 'collaborative_signals', 'growth_mindset']}}
...
async def get_emotion_summary(request: SessionSummaryRequest):
    ...
    return {'avg_confidence': summary.avg_confidence, 'confidence_trend': summary.confidence_trend, 'stress_peaks': summary.stress_peaks, 'total_filler_words': summary.total_filler_words, 'avg_eye_contact': summary.avg_eye_contact, 'avg_speech_rate_wpm': summary.avg_speech_rate_wpm, 'emotion_distribution': summary.emotion_distribution}
...
@app.get('/health')
async def health():
    return {'status': 'healthy', 'service': settings.service_name, 'port': settings.service_port, 'active_sessions': len(session_pipelines)}
