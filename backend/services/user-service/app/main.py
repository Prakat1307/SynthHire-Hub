...
async def get_subscription(
    ...
    if not sub:
        ...
        sub = Subscription(
            ...
            user_id=uuid.UUID(user_id),
            plan="free",
            sessions_limit=settings.free_sessions_per_month
        )
        db.add(sub)
        db.commit()
        db.refresh(sub)
...
async def get_recent_activity(
    ...
    try:
        ...
        for s in sessions:
            ...
            activities.append({
                "id": str(s.id),
                "type": "interview",
                "title": f"{type_val.replace('_', ' ').title()} Interview",
                "status": status_val,
                "timestamp": s.created_at.isoformat() if s.created_at else None,
                "score": s.overall_score,
                "link": f"/interview/report/{s.id}" if status_val == "completed" else f"/interview/{s.id}"
            })
    ...
    try:
        ...
        for g in jobs:
            ...
            activities.append({
                "id": str(g.application_id),
                "type": "job",
                "title": g.job.title if (hasattr(g, 'job') and g.job) else "Job Application",
                "status": status_val,
                "timestamp": g.applied_at.isoformat() if g.applied_at else None,
                "company": g.job.company if (hasattr(g, 'job') and g.job) else "External",
                "link": "/jobs"
            })
    except Exception as e:
    ...
    try:
        preps = db.query(PrepAnswerHistory).filter(
            PrepAnswerHistory.user_id == uid
        ).order_by(PrepAnswerHistory.created_at.desc()).limit(10).all()
        for p in preps:
            activities.append({
                "id": str(p.id),
                "type": "prep",
                "title": f"Prep Question Answered",
                "status": "completed",
    ...
    try:
        resumes = db.query(ResumeVersion).filter(
            ResumeVersion.user_id == uid
        ).order_by(ResumeVersion.created_at.desc()).limit(10).all()
        for r in resumes:
            activities.append({
                "id": str(r.id),
                "type": "resume",
                "title": r.label or "Resume Update",
                "status": "active" if r.is_active else "archived",
