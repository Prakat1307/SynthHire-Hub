...
class MongoDBClient:
    ...
    async def save_exchange(self, exchange: Dict[str, Any]):
        return await self.db.exchanges.insert_one({**exchange, 'created_at': datetime.utcnow()})

    async def get_session_exchanges(self, session_id: str) -> List[Dict[str, Any]]:
        cursor = self.db.exchanges.find({'session_id': session_id}).sort('exchange_number', 1)
        return await cursor.to_list(length=1000)

    async def save_whiteboard_snapshot(self, snapshot: Dict[str, Any]):
        return await self.db.whiteboard_snapshots.insert_one({**snapshot, 'created_at': datetime.utcnow()})

    async def save_emotional_frame(self, frame: Dict[str, Any]):
        return await self.db.emotional_frames.insert_one({**frame, 'created_at': datetime.utcnow()})

    async def get_session_emotions(self, session_id: str) -> List[Dict[str, Any]]:
        cursor = self.db.emotional_frames.find({'session_id': session_id}).sort('timestamp', 1)
    ...
    async def save_report_data(self, report: Dict[str, Any]):
        return await self.db.reports.insert_one({**report, 'created_at': datetime.utcnow()})
