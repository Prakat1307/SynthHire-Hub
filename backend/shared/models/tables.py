from sqlalchemy import Column, String, Boolean, DateTime, Integer, Float, ForeignKey, Text, Enum, JSON, Index
...
class TrainingData(Base):
    ...
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    __table_args__ = (Index('idx_training_split', 'split'),)
...
class MLModelRegistry(Base):
    ...
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    __table_args__ = (Index('idx_model_active', 'model_name', 'is_active'),)
...
class CompanyUser(Base):
    ...
    __table_args__ = (Index('idx_company_user_unique', 'company_id', 'user_id', unique=True),)
...
class ScoringProfile(Base):
    ...
    company_id = Column(UUID(as_uuid=True), ForeignKey('company_accounts.id', ondelete='CASCADE'), nullable=False, index=True)
    name = Column(String(255), nullable=False)
    weights = Column(JSON, nullable=False, default={'technical_correctness': 0.2, 'problem_solving': 0.2, 'communication': 0.15, 'code_quality': 0.15, 'system_design': 0.1, 'behavioral': 0.1, 'leadership': 0.05, 'culture_fit': 0.05})
    thresholds = Column(JSON, nullable=True, default={'technical_correctness': 50, 'communication': 40})
