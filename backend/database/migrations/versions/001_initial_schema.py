...
def upgrade():
    ...
    op.execute(
        "ALTER TABLE questions ADD COLUMN IF NOT EXISTS ideal_response_embedding vector(384)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_questions_embedding ON questions "
        "USING ivfflat (ideal_response_embedding vector_cosine_ops) WITH (lists = 50)"
    )
    
    op.execute(
        "ALTER TABLE session_exchanges ADD COLUMN IF NOT EXISTS candidate_response_embedding vector(384)"
    )
